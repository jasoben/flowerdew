...
{ 
		...
		where T : WorkflowStateObject
	{
		...
		{
			(_behaviorHost as hg.ApiWebKit.editor.EditorProgressionBehavior).Subscribe();
		}
		
		public void UnsubscribeEditorFromUpdates()
		{
			(_behaviorHost as hg.ApiWebKit.editor.EditorProgressionBehavior).Unsubscribe();
		}
		#endif
		...
		{		
			...
			{
				#if UNITY_EDITOR
				_behaviorHost = (hg.ApiWebKit.editor.EditorProgressionBehavior)Configuration.Bootstrap().AddComponent(typeof(hg.ApiWebKit.editor.EditorProgressionBehavior));
				#else
